...
export class DemoTableComponent implements OnInit {
  ...
  filtrarCurso(event: Event){
    ...
    this.dataSource.filterPredicate = function(curso: Curso, filtro: string){
      return curso.nombre.toLocaleLowerCase().includes(filtro.toLocaleLowerCase()); // filtra solo 1 tabla
    };
    this.dataSource.filter = valorObtenido.trim().toLowerCase(); // filtra todo
  }
